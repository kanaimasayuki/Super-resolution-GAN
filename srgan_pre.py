...
def main():
    ...
    hr_img = img
    imheight, imwidth, channels = hr_img.shape[:3]
    print("# Height: {}".format(imheight))
    print("# Width: {}".format(imwidth))

    # Network Setup
    model = Generator(input_shape=(imheight, imwidth, 3))
    model.build((None, imheight, imwidth, 3))
    model.load_weights(args.param)

    # Low-resolution Image
    lr_img = cv2.resize(hr_img, (int(imwidth/args.mag), int(imheight/args.mag)))
    lr_img = cv2.resize(lr_img, (imwidth, imheight))
    lr_img_s = lr_img
